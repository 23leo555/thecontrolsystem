-- Brief V2: formularz /reset przesyła wersję klauzuli zgód.
-- Przechowujemy ją przy leadzie jako dowód, na jakiej treści oparto zgodę.
alter table public.leads
  add column if not exists consent_version text;

-- Stara sygnatura (11 parametrów) przestaje być potrzebna — usuwamy, żeby
-- PostgREST nie miał dwóch przeciążeń i nie zgadywał, które wywołać.
drop function if exists public.upsert_lead(text,text,boolean,text,text,text,text,text,text,text,text);
...
begin
  ...
  insert into public.leads as l (
    first_name, email, marketing_consent, consent_at, consent_version,
    source_first, utm_source, utm_medium, utm_campaign, utm_content, utm_term,
    referrer, landing_path, lifecycle_status
  )
  values (
    p_first_name, lower(p_email), coalesce(p_marketing_consent, false), now(), p_consent_version,
  ...
  on conflict (email) do update set
    ...
    consent_at        = now(),
    consent_version   = coalesce(excluded.consent_version, l.consent_version),
...
revoke all on function public.upsert_lead(text,text,boolean,text,text,text,text,text,text,text,text,text)
